        ...
        <div class="row">
            @foreach ($project->tickets as $ticket)
                <div data-ticket-id="{{ $ticket->id }}"
                     class="ticket order col-md-12 @if ((int) \Input::get('selected_ticket') === (int) $ticket->id) show-form @endif">
                    <div class="ticket-wrapper">
                        <form action="{{ url('/tickets/') }}/{{ $ticket->id }}/orders/form" method="post"
                              class="ticket-body row display-table">
                            ...
                            <div class="col-md-9 display-cell">
                                <div class="form-group stop-propagation">
                                    <div class="input-group col-sm-4">
                                        <input class="ticket_audiences_limit" type="hidden"
                                               value="{{ $ticket->audiences_limit }}"/>
                                        <input class="available_ticket_count" type="hidden"
                                               value="{{ $ticket->audiences_limit - $ticket->audiences_count }}"/>
                                        @if ($project->type === 'funding')
                                            <input name="request_price" type="number" class="form-control" max="2500000"
                                                   value="{{ $ticket->price }}" min="{{ $ticket->price }}"/>
                                        ...
                                        @else
                                            ...
                                            <input name="ticket_count" type="number" class="form-control ticket_count"
                                                   value="1" min="1"/>
