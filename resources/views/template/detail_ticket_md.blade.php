    ...
    @if(!$project->isEventTypeCrawlingEvent())
      <div class="detail_calender_container">
        <h4 style="font-weight:600; margin-top:0px">
          TICKET <span class="detail_date_sub_title"> | @if($project->isEventSubTypeSandBox()) {{ $project->getConcertDateFormatted() }} @elseif($project->isEventTypeInvitationEvent()) 신청 가능날짜 @else 티켓팅 가능날짜 @endif: {{ $project->getConcertDateFormatted() }}</span>
        </h4>
