...
@foreach ($projects as $project)
    @if ($i % 3 === 0)
        <div class="item @if ($i === 0) active @endif">
            <div class="row">
    @endif
                <div class="@if (!isset($colOnly)) col-xs-12 col-sm-12 col-md-6 col-md-4 @endif project-grid">
                    <div class="project-grid-wrapper">
                        <div>
                            @if ($project->isPublic())
                                <a href="{{ url('/projects') }}/{{ $project->id }}">
                                    <div class="bg-base project-thumbnail"
                                         style="background-image: url('{{ $project->getPosterUrl() }}')">
                                        <div class="white-mask"></div>
                                    </div>
                                </a>
                                <h4 class="text-ellipsize project-title"><a
                                            href="{{ url('/projects') }}/{{ $project->id }}">{{ $project->title }}</a></h4>
                                    @else
                                        ...
                                        <h4 class="text-ellipsize project-title"><a
                                                    href="{{ url('/projects') }}/form/{{ $project->id }}">{{ $project->title }}</a></h4>
                                            @endif
                            ...
                            <div class="project-progress-wrapper">
                                <div class="progress">
                                    <div class="progress-bar" role="progressbar"
                                         aria-valuenow="{{ $project->getProgress() }}" aria-valuemin="0" aria-valuemax="100"
